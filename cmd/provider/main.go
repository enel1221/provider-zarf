...
import (
	...
	"github.com/crossplane/provider-zarf/apis"
	"github.com/crossplane/provider-zarf/apis/v1alpha1"
	zarf "github.com/crossplane/provider-zarf/internal/controller"
	"github.com/crossplane/provider-zarf/internal/features"
	"github.com/crossplane/provider-zarf/internal/version"
)
...
func main() {
	var (
		...
		pollInterval            = app.Flag("poll", "How often individual resources will be checked for drift from the desired state").Default("2m").Duration()
		pollStateMetricInterval = app.Flag("poll-state-metric", "State metric recording interval").Default("10s").Duration()
	...
	mgr, err := ctrl.NewManager(ratelimiter.LimitRESTConfig(cfg, *maxReconcileRate), ctrl.Options{
		...
		LeaderElectionID:           "crossplane-leader-election-provider-zarf",
		LeaderElectionResourceLock: resourcelock.LeasesResourceLock,
		LeaseDuration:              func() *time.Duration { d := 60 * time.Second; return &d }(),
		RenewDeadline:              func() *time.Duration { d := 50 * time.Second; return &d }(),
	})
	kingpin.FatalIfError(err, "Cannot create controller manager")
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add Zarf APIs to scheme")
	...
	if *enableChangeLogs {
		...
		clo := controller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-zarf:%s", version.Version))),
		}
		o.ChangeLogOptions = &clo
	}

	kingpin.FatalIfError(zarf.Setup(mgr, o), "Cannot setup Zarf controllers")
